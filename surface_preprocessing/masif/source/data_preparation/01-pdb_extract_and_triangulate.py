...
if HAS_PYMESH:
    ...
    surf_vertices = regular_mesh.vertices
    surf_faces = regular_mesh.faces
else:
    surf_vertices = np.array(vertices2)
    surf_faces = np.array(faces2)
vertex_normal = compute_normal(surf_vertices, surf_faces)
if HAS_PYMESH and masif_opts['use_hbond']:
    vertex_hbond = assignChargesToNewMesh(surf_vertices, vertices1, vertex_hbond, masif_opts)
if HAS_PYMESH and masif_opts['use_hphob']:
    vertex_hphobicity = assignChargesToNewMesh(surf_vertices, vertices1, vertex_hphobicity, masif_opts)
if masif_opts['use_apbs']:
    print(f"Computing APBS...")
    vertex_charges = computeAPBS(surf_vertices, out_filename1+".pdb", out_filename1, mol2_file)
    print(f"APBS done!")
iface = np.zeros(len(surf_vertices))
if 'compute_iface' in masif_opts and masif_opts['compute_iface']:
    ...
    if HAS_PYMESH:
        ...
        v3 = np.array(v3)
    kdt = KDTree(v3)
    d, r = kdt.query(surf_vertices)
    d = np.square(d)
    assert(len(d) == len(surf_vertices))
    iface_v = np.where(d >= 2.0)[0]
    iface[iface_v] = 1.0
    save_ply(out_filename1+".ply", surf_vertices, surf_faces, normals=vertex_normal, charges=vertex_charges, normalize_charges=True, hbond=vertex_hbond, hphob=vertex_hphobicity, iface=iface)
else:
    save_ply(out_filename1+".ply", surf_vertices, surf_faces, normals=vertex_normal, charges=vertex_charges, normalize_charges=True, hbond=vertex_hbond, hphob=vertex_hphobicity)
